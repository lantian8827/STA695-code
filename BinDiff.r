BinRatio <- function(n1, x1, n2, x2, step=0.01, initStep=0.01, level=3.84){
  
  if(x1 >= n1) stop("all success?")
  
  
  
  MLE <- (x2/n2)-(x1/n1)   #### recall we are to find CI for p2/p1, not p1/p2.
  ...
  Theta <- function(theta, n1=n1, x1=x1, n2=n2, x2=x2)
  {
    ...
    upBD <- min(1-theta-EPS,1-EPS)
    lbBD <- max(EPS,-theta+EPS)
    
    temp <- optimize(f = llr, lower = lbBD, upper = upBD, n1 = n1, x1 = x1, n2 = n2, x2 = x2,theta = theta)
